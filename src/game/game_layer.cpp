...
namespace RealmFortress
{
    GameLayer::GameLayer()
        : Layer("GameLayer")
    {
        mBasicShader = mShaderLibrary.Load("assets/shaders/basic.glsl");
        mInstanceShader = mShaderLibrary.Load("assets/shaders/instanced.glsl");
        mGhostBuildingShader = mShaderLibrary.Load("assets/shaders/ghost_building.glsl");
    ...
    void GameLayer::OnUpdate(Timestep ts)
    {
        ...
        mMap.OnUpdate(mCameraController->GetCamera().GetPosition());
        ...
        mMap.Draw(mInstanceShader);

        mBasicShader->Bind();
        mBasicShader->SetMat4("uViewProjection", Renderer::GetViewProjectionMatrix());

        for (const auto& building : BuildingManager::Get().GetAllBuildings())
        {
            if (building && building->GetModel())
            {
                building->GetModel()->Draw(mBasicShader, building->GetTransform());
            }
        }

        std::unordered_set<Coordinate> lifted_tiles;

        if (mGameMode == GameMode::Building)
        {
            DrawGhostBuilding();
        }
        else if (mGameMode == GameMode::Normal)
        {
    ...
    void GameLayer::OnImGuiRender()
    {
        ...
        if (UI_PANEL_IS_OPEN(mUIPanelFlags, UIPanelInspect))
        {
            DrawInspectPanel();
        }
    }
            ...
            else
            {
                if (mSelection.HasHover())
                {
                    ...
                    if (mInspectedBuilding)
                        UI_PANEL_OPEN(mUIPanelFlags, UIPanelInspect);
                    else
                        UI_PANEL_CLOSE(mUIPanelFlags, UIPanelInspect);
                }
                else
                {
    ...
    void GameLayer::DrawGhostBuilding()
    {
        ...
        if (model)
        {
            mGhostBuildingShader->Bind();
            mGhostBuildingShader->SetMat4("uViewProjection", Renderer::GetViewProjectionMatrix());

            glm::vec3 color = can_place ? glm::vec3(0.0f, 1.0f, 0.0f) : glm::vec3(1.0f, 0.0f, 0.0f);
            mGhostBuildingShader->SetFloat3("uHighlightColor", color);
            mGhostBuildingShader->SetFloat("uPulseTime", mTime);
            mGhostBuildingShader->SetFloat("uHighlightIntensity", 0.3f);

            glm::mat4 transform = glm::mat4(1.0f);
            transform = glm::translate(transform, coord.ToWorldPosition());

            model->Draw(mGhostBuildingShader, transform);
        }
    }
    ...
    constexpr ImVec2 InspectPanel_SizeScale(0.25f, 0.55f);
    ...
    void GameLayer::DrawActionBar(ImVec2* out_pos, ImVec2* out_size)
    {
        ...
        ImGui::Begin("ActionBar", nullptr, flags);
        {
            ...
            *out_size = ImGui::GetWindowSize();
            ...
            ImGui::PushStyleColor(ImGuiCol_Button, IM_COL32(40, 40, 34, 255));
            ImGui::PushStyleColor(ImGuiCol_ButtonActive, IM_COL32(40, 40, 34, 255));
            ImGui::PushStyleColor(ImGuiCol_ButtonHovered, IM_COL32(60, 60, 54, 255));
    ...
    void GameLayer::DrawInspectPanel()
    {
        if (!mInspectedBuilding) return;

        ImGuiViewport* viewport = ImGui::GetMainViewport();

        ImVec2 pos(viewport->Pos.x + viewport->Size.x, viewport->Pos.y + viewport->Size.y);
        ImGui::SetNextWindowPos(pos, ImGuiCond_Always, ImVec2(1.0f, 1.0f));

        ImVec2 size(viewport->Size * InspectPanel_SizeScale);
        ImGui::SetNextWindowSize(size, ImGuiCond_Always);
        ...
        ImGui::Begin("InspectPanel", nullptr, window_flags);
        {
            ImGuiChildFlags child_flags = ImGuiChildFlags_AlwaysUseWindowPadding;

            ImGui::PushStyleColor(ImGuiCol_ChildBg, IM_COL32(87, 87, 61, 255));
            ImGui::BeginChild("InspectPanelBG", ImVec2(0.0f, 0.0f), child_flags);
            {
                ...
                // Building Specific
                mInspectedBuilding->OnInspected();

                // Action Buttons
                ImGui::Spacing();
                if (ImGui::Button("Demolish", ImVec2(-1, 30)))
                {
                    BuildingManager::Get().RemoveBuilding(mInspectedBuilding->GetCoordinate(), mMap);
                    mInspectedBuilding = nullptr;
                    UI_PANEL_CLOSE(mUIPanelFlags, UIPanelInspect);
                }
            }
